...
+function ($) {
  "use strict";
        ...
        if (defaults.modalTemplate) {
            ...
            if (params.buttons && params.buttons.length > 0) {
                for (var i = 0; i < params.buttons.length; i++) {
                    if(params.buttons[i].fill){
                        buttonsHTML += '<span class="modal-button button button-fill">' + params.buttons[i].text + '</span>';
                    }
                    else if(params.buttons[i].icon){
                        buttonsHTML += '<span class="modal-button button-icon"><i class="icon-close"></i></span>';
                    }
                    else{ 
            ...
            var fillButtons = params.fillButtons ? 'modal-buttons-fill' : '';
            modalHTML = '<div class="modal ' + noButtons + '"><div class="modal-inner">' + (titleHTML + textHTML + afterTextHTML) + '</div><div class="modal-buttons ' + verticalButtons + ' ' + fillButtons + '">' + buttonsHTML + '</div></div>';
        }
    ...
    $.prompt = function (title, text, placeholder, callbackOk, callbackCancel) {
        ...
        return $.modal({
            title: title || '',
            text: text || '',
            afterText: '<input type="text" class="modal-text-input" placeholder = "' + placeholder + '">',
            buttons: [
                {
            ...
            onClick: function (modal, index) {
                ...
                if (index === 1 && callbackCancel) callbackCancel($(modal).find('.modal-text-input').val());
            }
        });
    };
    $.modalPassword = function (text, placeholder, callbackOk, callbackCancel) {
        if (typeof placeholder === 'function') {
            callbackCancel = arguments[2];
            callbackOk = arguments[1];
        ...
        return $.modal({
            text: text || '',
            title: '',
            afterText: '<input type="password" class="modal-text-input" placeholder="' + placeholder + '" name="modal-password">',
            buttons: [
                {
                    text: defaults.modalButtonOk,
                    fill: true
            ...
            onClick: function (modal, index) {
                ...
                if (index === 0 && callbackOk) callbackOk(password);
                if (index === 1 && callbackCancel) callbackCancel(password);
            }
        });
    };
    $.showPreloader = function (text) {
        var html = '<div class="preloader-indicator-overlay"></div>' +
            '<div class="preloader-indicator-modal loader">' +
            '<span class="preloader preloader-white"></span>' +
            '<p>' + (text || defaults.modalPreloaderTitle) + '</p>' +
            '</div>';
        $(defaults.modalContainer).append(html);
    };
    $.hidePreloader = function () {
        $('.preloader-indicator-overlay, .preloader-indicator-modal.loader').remove();
    };
    ...
    $.toast = function(msg, time, type) {
        ...
        var $toast = $("<div class='modal toast "+type+"'>"+msg+"</div>").appendTo(document.body);
        $.openModal($toast);
        setTimeout(function() {
            $.closeModal($toast);
        }, time || 2000);
