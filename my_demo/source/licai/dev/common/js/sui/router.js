...
+ function($) {
    ...
    Router.prototype.getHash = function(url){
        var hash = "", temp = "";
        if(!url) return hash;
		temp = url.match(/\#([A-Za-z0-9-_\?=]*)/i);
		if (temp && temp.length > 1) {
			hash = temp[1];
		} else {
			temp = url.match(/[\?|\&]link([A-Za-z0-9-_\?=]*)\&*/i);
			if(temp && temp.length > 1) temp = temp[1].split('=');
			if(temp && temp.length > 1) hash = temp[1];
		}
		if(!temp || temp.length < 2) {
			temp = [];
			temp = url.split('\/');
			hash = temp[temp.length-1];
			hash = hash.match(/([A-Za-z0-9_\-]*)/i);
    ...
    Router.prototype.reloadPage = function(url, back) {
        this.getPage(url, function(page) {
            ...
            if (back) {
                ...
                this.animatePages(this.getCurrentPage(), page.eq(0));
            }
    ...
    Router.prototype._forward = function() {
        ...
        this.setCurrentStateID(h.id);
        this.animatePages(currentPage, newPage.eq(0));
    }
